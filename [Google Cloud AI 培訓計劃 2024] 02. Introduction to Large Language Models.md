...
**大型語言模型可用在許多工作上，包括寫作、翻譯和程式設計。 [check]**
...
**這類模型可生成猶如真人撰寫的文字，協助處理內容創作、撰文輔助和自動製作摘要等工作。 [check]**
...
**大型語言模型是一種人工智慧 (AI)，可生成猶如真人撰寫的文字。這類模型由龐大的語料和程式碼資料集訓練而成，能處理寫作、翻譯和程式設計等許多工作。 [check]**
...
**4. 使用大型語言模型可能會遇到哪些挑戰？請選擇三個選項。** 

模型建立完畢後，只有提供新資料給模型時，模型才會變更。

**訓練成本可能相當高。 [check]**

**這類模型可能遭人用來生成有害內容。 [check]**
